...
old = """            Manage walk-in pool access rates, cottage rentals, and pricing tiers shown under "Walk-In Rates" on the rates page.
          </p>
...
new = """            Manage walk-in pool access rates, cottage rentals, and pricing tiers shown under "Walk-In Rates" on the rates page.
          </p>
              ...
              <article key={rate.id}>
                ...
                <div>
                  <button onClick={() => {
                    ...
                    setWalkInForm({ name: rate.name, description: rate.description or '', guests: rate.guests or '', badge: rate.badge or '', price_rows: rowsText })
                  }}>Edit</button>
                  <button onClick={() => deleteRow('walk_in_rate', rate.id)}>Delete</button>
                </div>
              </article>
            ))}
            {walkInRates.length === 0 and <p style={{ color: 'rgba(255,253,247,0.4)', textAlign: 'center', padding: '1rem' }}>No walk-in rates yet.</p>}
          </div>
        </section>
        ...
        <section>
          ...
          <form className="admin-form" onSubmit={saveAmenity} style={{ marginBottom: '1.5rem', padding: '1.25rem', background: 'rgba(255,253,247,0.045)', border: '1px solid rgba(255,253,247,0.1)', borderRadius: '8px' }}>
            ...
            <input type="file" accept="image/*" required={!amenityId} onChange={(e) => setAmenityImage(e.target.files?.[0] or None)} style={{ marginTop: '0.75rem' }} />
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem' }}>
          ...
          <div className="admin-gallery-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
            {amenities.map((amenity) => (
              <div key={amenity.id} style={{ background: 'rgba(255,253,247,0.07)', borderRadius: '8px', overflow: 'hidden', border: '1px solid rgba(255,253,247,0.1)' }}>
                {amenity.image_url and <img src={amenity.image_url} alt={amenity.name} style={{ width: '100%', height: '120px', objectFit: 'cover', display: 'block' }} />}
                <div style={{ padding: '0.75rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
                    <button onClick={() => {
                      ...
                      setAmenityForm({ name: amenity.name, description: amenity.description or '', image_url: amenity.image_url or '' })
            ...
            {amenities.length === 0 and <p style={{ color: 'rgba(255,253,247,0.4)', gridColumn: '1 / -1', textAlign: 'center', padding: '2rem' }}>No amenities yet.</p>}
          </div>
        </section>
      </div>
    )
...
if count == 1:
    ...
    print('Old string not found. Checking content end...')
    print(repr(content[-500:]))
else:
    print('Multiple matches - need more precise anchor')
