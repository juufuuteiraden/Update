...
old = """            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem' }}>
...
new = """            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem' }}>
...
if count == 1:
    ...
    import re
    # Find the walkIn form submit button with Cancel
    idx = content.find('<button type="submit" disabled={loading}>Cancel</button>')
    print(f'Found Cancel button at index: {idx}')
    if idx > 0:
        # Get context
        start = content.rfind('<div style={{ display:', 0, idx)
        end = content.find('  )\n  }', idx) + 20
        if end < idx:
            end = len(content)
        print(f'Context start={start}, end={end}')
        snippet = content[start:end]
        print(f'Snippet to replace:')
        print(repr(snippet))
else:
    print('Multiple matches - need more precise context')
