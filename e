...
export default function Packages({
  ...
  useEffect(() => {
    ...
    const loadPackages = async () => {
      const { data, error } = await supabase.from('packages').select('id,name,price,inclusions,highlighted');
      if (!isMounted || error || !data?.length) return;
      setPackages(data.map((pkg: PackageItem, index) => {
        const fallback = fallbackPackages[index % fallbackPackages.length];
        return { ...fallback, id: pkg.id, title: pkg.name, price: pkg.price, badge: pkg.highlighted ? 'Featured' : fallback.badge, includes: Array.isArray(pkg.inclusions) && pkg.inclusions.length ? pkg.inclusions : fallback.includes, highlighted: pkg.highlighted };
      }));
    };
    loadPackages();
      ...
      <AdminModal
        ...
        footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
            <button type="button" className="rates-admin-modal-cancel" onClick={() => setPackageModalOpen(false)} disabled={savingPackage}>Cancel</button>
